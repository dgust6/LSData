...
open class SwitchDataSource<DS1: DataSource, DS2: DataSource>: DataSource where DS2.Output == DS1.Output, DS1.Parameter == DS2.Parameter, DS1.OutputError == DS2.OutputError {
...
public extension DataSource {
    ...
    ///
    /// Creates `SwitchDataSource`.
    func switchOnCondition<DS2: DataSource>(to dataSource: DS2, condition: @escaping (Output) -> Bool) -> SwitchDataSource<Self, DS2> where Self.Output == DS2.Output, Self.Output == DS2.OutputError, Self.Parameter == DS2.Parameter {
        SwitchDataSource<Self, DS2>(baseSource: self, switchSource: dataSource, condition: condition)
    }
}
...
public extension DataSource where Output: Collection {
    ...
    ///
    /// Creates `SwitchDataSource`.
    func switchOnEmpty<DS2: DataSource>(to dataSource: DS2) -> SwitchDataSource<Self, DS2> where Self.Output == DS2.Output, Self.Output == DS2.OutputError, Self.Parameter == DS2.Parameter {
        SwitchDataSource<Self, DS2>(baseSource: self, switchSource: dataSource, condition: { $0.isEmpty })
    }
}
